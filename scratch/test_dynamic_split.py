...
def test_extract_card_centered_dynamic(analyzer, card_img):
    ...
    # Crop the top 60% of the card
    top_60 = card_img[0:int(h*0.60), :]
    
    # Convert to grayscale and threshold to binary (inverted: ink is 255)
    gray = cv2.cvtColor(top_60, cv2.COLOR_BGR2GRAY)
    _, binary = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY_INV)
    
    # Compute horizontal projection (row sums of ink pixels)
    row_sums = np.sum(binary > 0, axis=1)
    
    # We want to find the gap between the rank (at the top) and the suit (at the bottom)
    # The rank is in the top portion (y < 40% of card height)
    # The suit is in the bottom portion (y > 40% of card height)
    y_40 = int(h * 0.40)
    
    # Find the row index in the region [y_40 - 5, y_40 + 10] with the minimum ink sum
    search_start = max(10, y_40 - 8)
    search_end = min(top_60.shape[0] - 2, y_40 + 8)
    
    best_y = y_40
    min_sum = float('inf')
    for y in range(search_start, search_end):
        # We can look at a window of 3 rows to make it robust
    ...
    # Save debug rank crops
    cv2.imwrite(f"/Users/admin/.gemini/antigravity-ide/brain/99eec12b-56cc-4eb2-b9c6-920fc66c5416/rank_dynamic_{best_y}.png", rank_crop)
...
def main():
    ...
    for i, c in enumerate(contours):
        ...
        if 1500 < area < 4000:
            ...
            rank, suit = test_extract_card_centered_dynamic(analyzer, card_crop)
