...
def test_pipeline():
    ...
    # We will use Thresh 200 (Invert=True, PSM=11), Thresh 220 (Invert=True, PSM=6), Thresh 180 (Invert=True, PSM=6)
    runs = [
        (200, True, 11),
        (220, True, 6),
        (180, True, 6)
    ...
    for rank_hint, cx, cy, conf in unique_candidates:
        # Determine Card Crop coordinates using the new vertical offset (cy - 33)
        card_w, card_h = 42, 66
        card_x1 = int(cx - 21)
        card_y1 = int(cy - 33)
        ...
        # Fallback to rank_hint if rank is None
