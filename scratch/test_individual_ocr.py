...
def main():
    ...
    blocks = [
        ...
        (19, 75, 114, 85, "spade", 4),
        (142, 75, 114, 85, "heart", 4),
    ...
    for idx, (bx, by, bw, bh, suit, num_cards) in enumerate(blocks):
        ...
        for i in range(num_cards):
            ...
            # Crop rank: y = by+2..by+28, x = cx+3..cx+25
            rank_crop = img[by+2 : by+28, cx+2 : cx+24]
            
            # Print average color to ensure we are on a white card
            mean_val = np.mean(rank_crop)
            
            # Run OCR with various preprocessing
            detected_rank = None
            ...
            print(f"    Card {i+1} at cx={cx} (mean={mean_val:.1f}): Rank={detected_rank or '?'}")
